    ...
    <div class="display">
                ...
                <table class="tableView" id="tableTrucks">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 33%;" onclick="sortTable(1);arrowsTable(1);" id="1">{{ __("Desde") }}</th>
                            <th style="width: 33%;" onclick="sortTable(2);arrowsTable(2);" id="2">{{ __("Hasta") }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($vehiculos as $vehiculo)
                            <tr>
                                ...
                                <td>{{ $vehiculo->hasta != null ? \Carbon\Carbon::parse($vehiculo->hasta)->format('d/m/y H:i') : __('Conduciendo') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="editContainer">
                ...
                @else
                    ...
                    <br>
                        <form action="{{ route('conducen.desde') }}" method="POST">
                            ...
                            <div>
                                <label for="matricula" style="color: white; font-size: 2vh; font-weight: 500">{{ __("Vehiculo") }}:</label>
            ...
            <form action="{{ route('camioneros.update', $camionero) }}" method="POST">
                ...
                @method('PATCH')
                <div>
                    <label for="nombre" style="color: white; font-size: 2vh; font-weight: 500">{{ __("Nombre") }}: </label>
